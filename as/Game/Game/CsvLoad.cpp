#include "CsvLoad.h"
#include <fstream>		// ファイル入出力ストリームを使うためのヘッターファイル
#include <cassert>		// assertを使うためのヘッターファイル

std::vector<std::vector<std::string>> CsvLoad::LoadFile(std::string path)
{
	// ファイルの読み込み
...
std::vector<std::string> CsvLoad::Split(const std::string& str, const char cutChar)
{
